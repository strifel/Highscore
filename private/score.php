...
function getScore($game) {
    ...
    $score =  file_get_contents("private/score/" . $game . ".score");
    return intval($score);
}

function gameExists($game) {
    if (!secureGameName($game)) return false;
    return file_exists("private/score/" . $game . ".score");
}

function setScore($game, $score) {
    if (!secureGameName($game)) return false;
    if (!is_int($score)) return false;
    return file_put_contents("private/score/" . $game . ".score", strval($score));
}
